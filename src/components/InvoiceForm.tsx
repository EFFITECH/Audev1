...
export const InvoiceForm: React.FC<InvoiceFormProps> = ({
  ...
  const [formData, setFormData] = useState<Invoice>({
    ...
    amount: invoice?.amount || (order ? order.totalTTC : 0),
    paymentStatus: invoice?.paymentStatus || 'non payé',
    paymentDueDate: invoice?.paymentDueDate || (order ? order.paymentDueDate : ''),
    notes: invoice?.notes || '',
  ...
  const validate = (): boolean => {
    ...
    if (!invoice && !file && !formData.fileUrl) {
      newErrors.fileUrl = 'Un fichier de facture est requis';
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validate()) {
      ...
      const updatedInvoice = {
        ...formData,
        fileUrl: file ? URL.createObjectURL(file) : formData.fileUrl,
      };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="amount" className="block text-sm font-medium text-gray-700 dark:text-dark-text mb-1">
            Montant (€) *
          </label>
          <input
            type="number"
      ...
      <div>
        ...
        <input
          ...
          className={`w-full px-3 py-2 border rounded-md dark:bg-dark-input dark:text-dark-text dark:border-dark-border ${
            errors.fileUrl ? 'border-red-500 dark:border-red-500' : 'border-gray-300'
          }`}
        />
        {errors.fileUrl && <p className="mt-1 text-sm text-red-500">{errors.fileUrl}</p>}
        {formData.fileUrl && (
          <p className="mt-1 text-sm text-gray-500 dark:text-dark-muted">
            Fichier actuel: {formData.fileUrl.split('/').pop()}
          </p>
        )}
      </div>
